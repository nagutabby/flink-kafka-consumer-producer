...
def get_kafka_producer(source, metrics):
    if metrics == 'average':
        if 'SCD41' in source:
            if 'co2' in source:
                topic = f'{TOPIC_PREFIX}-analytics-s2410064_SCD41_avg-co2'
            elif 'humidity' in source:
                topic = f'{TOPIC_PREFIX}-analytics-s2410064_SCD41_avg-humidity'
            elif 'temperature' in source:
                topic = f'{TOPIC_PREFIX}-analytics-s2410064_SCD41_avg-temperature'
        elif 'BMP180' in source:
            if 'temperature' in source:
                topic = f'{TOPIC_PREFIX}-analytics-s2410064_BMP180_avg-temperature'
            elif 'air_pressure' in source:
                topic = f'{TOPIC_PREFIX}-analytics-s2410064_BMP180_avg-air_pressure'
    elif metrics == 'max':
        if 'SCD41' in source:
            if 'co2' in source:
                topic = f'{TOPIC_PREFIX}-analytics-s2410064_SCD41_max-co2'
            elif 'humidity' in source:
                topic = f'{TOPIC_PREFIX}-analytics-s2410064_SCD41_max-humidity'
            elif 'temperature' in source:
                topic = f'{TOPIC_PREFIX}-analytics-s2410064_SCD41_max-temperature'
        elif 'BMP180' in source:
            if 'temperature' in source:
                topic = f'{TOPIC_PREFIX}-analytics-s2410064_BMP180_max-temperature'
            elif 'air_pressure' in source:
                topic = f'{TOPIC_PREFIX}-analytics-s2410064_BMP180_max-air_pressure'
    elif metrics == 'min':
        if 'SCD41' in source:
            if 'co2' in source:
                topic = f'{TOPIC_PREFIX}-analytics-s2410064_SCD41_min-co2'
            elif 'humidity' in source:
                topic = f'{TOPIC_PREFIX}-analytics-s2410064_SCD41_min-humidity'
            elif 'temperature' in source:
                topic = f'{TOPIC_PREFIX}-analytics-s2410064_SCD41_min-temperature'
        elif 'BMP180' in source:
            if 'temperature' in source:
                topic = f'{TOPIC_PREFIX}-analytics-s2410064_BMP180_min-temperature'
            elif 'air_pressure' in source:
                topic = f'{TOPIC_PREFIX}-analytics-s2410064_BMP180_min-air_pressure'
...
create_data_stream_and_sink_back(f'{TOPIC_PREFIX}-SCD41-co2')
create_data_stream_and_sink_back(f'{TOPIC_PREFIX}-SCD41-humidity')
create_data_stream_and_sink_back(f'{TOPIC_PREFIX}-SCD41-temperature')
create_data_stream_and_sink_back(f'{TOPIC_PREFIX}-BMP180-temperature')
create_data_stream_and_sink_back(f'{TOPIC_PREFIX}-BMP180-air_pressure')
