...
class CustomProcessWindowFunction(ProcessWindowFunction[tuple, tuple, str, TimeWindow]):
    def process(self, key, context, elements):
        ...
        for primary_source, values in dict_primary_sources.items():
            if 'i483-sensors-s2410064' in primary_source:
...
def get_primary_sink(primary_source):
    matched_object = re.match(r'i483-sensors-s2410064-([a-zA-Z0-9]+)-([a-z0-9_]+)', primary_source)
    sensor = matched_object.group(1)
    data_type = matched_object.group(2)

    PRIMARY_SINK_PREFIX = 'i483-sensors-s2410064-analytics-s2410064'
    primary_sink_average = f'{PRIMARY_SINK_PREFIX}_{sensor}_avg-{data_type}'
    primary_sink_max = f'{PRIMARY_SINK_PREFIX}_{sensor}_max-{data_type}'
    primary_sink_min = f'{PRIMARY_SINK_PREFIX}_{sensor}_min-{data_type}'
